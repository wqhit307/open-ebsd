...
class Dataset {
	...
	void init_params(Callback *c = NULL);
	int init_alignment();
	...
	Callback *callback;				//!< Callback for Python interface

	Dataset(const char name[], int dims[], float steps[], Callback *c = NULL);
	Dataset(const char name[], Callback *c = NULL);
